...
from PyQt6.QtWidgets import (
    QApplication, QWidget, QLabel, QPushButton, QSlider, QTabWidget, QTextEdit, QTextBrowser,
    QMenu, QMenuBar, QSplitter, QToolButton, QStatusBar, QHBoxLayout, QVBoxLayout, QFormLayout,
    QSizePolicy
)
        ...
        try:
            while response is None:
                ...
                if 'error' in response:
                    self.parent.status.setStyleSheet('color:red;')
                    self.parent.clear_input()
                    self.parent.status.showMessage(response['error']['user_message'])
                    return
                else:
                    self.parent.status.setStyleSheet('color: green;')
                    ...
                    self.parent.conversation_window.insertHtml('<p style="color:#fd9620"> <strong>[AI]:</strong><br>')
...
class AIAssistant(QWidget):
    ...
    def init_ui(self):
        ...
        # add sliders
        self.max_tokens = QSlider(Qt.Orientation.Horizontal, minimum=10, maximum=4096, singleStep=500,
                                  pageStep=500, value=200, toolTip='Maximum token ChatGPT can consume')
    ...
    def zoom_in(self):
        ...
        # increase font size only when the current size is less than 30 pixels
    ...
    def zoom_out(self):
        font = self.message_input.font()
        # decrease font size only when the current size is greater than 5 pixels
        if font.pixelSize() > 5:
            self.message_input.setStyleSheet('font-size: {0}px'.format(font.pixelSize() - 2))
            self.conversation_window.setStyleSheet('font-size: {0}px;'.format(font.pixelSize() - 2))
...
class AppWindow(QWidget):
    ...
    def init_menu(self):
        ...
        file_menu.addAction('&Save log to DB', self.save_conversation_log_to_db)
    ...
    def save_output(self):
        ...
        with open('{0}_Chat Log.txt'.format(timestamp), 'w', encoding='UTF-8') as f:
            f.write(conversation_window_log)
